...
public struct EZLoadingActivity {
    ...
    public struct Settings {
        ...
        */
        ...
        public static var FontName = "Lato-Regular"
    ...
    public static func show(text: String, disableUI: Bool) {
        if instance != nil  {
...
//    }
    ...
    public static func showOnController(text: String, disableUI: Bool, controller:UIViewController) -> Bool{
        ...
        instance = LoadingActivity(text: text, disableUI: disableUI)
        DispatchQueue.main.async() {
            instance?.showLoadingWithController(controller: controller);
        }
    ...
    public static func hide(success: Bool? = nil, animated: Bool = false) {
        if instance == nil {
            print("EZLoadingActivity: You don't have an activity instance")
            
        }
        
        if hidingInProgress != false {
        ...
        if !Thread.current.isMainThread {
            DispatchQueue.main.async() {
                ...
                topMostController!.view.alpha = 1.0
            }
        } else {
            instance?.hideLoadingActivity(success: success, animated: animated)
            topMostController!.view.alpha = 1.0
...
//        let time = DispatchTime.now(dispatch_time_t(DISPATCH_TIME_NOW), Int64(seconds * Double(NSEC_PER_SEC)))
...
//    }
    ...
    private class LoadingActivity: UIView {
        ...
        func showLoadingActivity() {
            ...
            topMostController!.view.alpha = 0.5
        ...
        func createShadowPath() -> UIBezierPath {
            ...
            myBezier.move(to: CGPoint(x: -2, y: -2))
            myBezier.addLine(to: CGPoint(x: frame.width + 2, y: -2))
            myBezier.addLine(to: CGPoint(x: frame.width + 2, y: frame.height + 2))
            myBezier.addLine(to: CGPoint(x: -2, y: frame.height + 2))
            myBezier.close()
            return myBezier
        }
            ...
            if animated {
                ...
                UIView.animate(withDuration: animationDuration, animations: {
                    self.icon.alpha = 1
                        ...
                        self.callSelectorAsync(selector: #selector(UIView.removeFromSuperview), delay: animationDuration)
                        instance = nil
